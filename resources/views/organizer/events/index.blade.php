...
<header class="flex justify-end items-center mb-8">
    <a href="{{ route('organizer.events.create') }}" class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition flex items-center gap-2">
...
<div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
    ...
    <div class="px-8 py-6 bg-slate-50/50 border-b flex flex-wrap gap-4 items-center">
        <form method="GET" action="{{ route('organizer.events.index') }}" class="flex-1 flex gap-2">
        ...
        <form method="GET" action="{{ route('organizer.events.index') }}" class="flex gap-2">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        <div class="flex justify-end gap-2">
                            ...
                            <!-- Edit -->
                            <a href="{{ route('organizer.events.edit', $event->id) }}" class="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition" title="Edit">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </a>
                            <!-- Delete -->
                            <form action="{{ route('organizer.events.destroy', $event->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus event {{ $event->title }}?');">
                                @csrf
                                @method('DELETE')
