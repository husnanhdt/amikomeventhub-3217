...
<div class="max-w-7xl mx-auto px-6 py-12">
    ...
    <div class="bg-gradient-to-r from-indigo-600 to-purple-700 rounded-3xl p-8 md:p-12 text-white shadow-xl mb-8 relative overflow-hidden">
        <div class="relative z-10 flex flex-col md:flex-row items-center gap-8">
            <!-- ✅ LOGO / Avatar (UPDATED) -->
            <div class="w-32 h-32 bg-white rounded-full flex items-center justify-center text-indigo-700 font-black text-4xl shadow-lg border-4 border-indigo-200 overflow-hidden">
                @if($partner->logo)
            ...
            <div class="bg-white rounded-3xl shadow-lg p-6 border border-slate-100 sticky top-32">
                ...
                <div class="space-y-4">
                    ...
                    <a href="{{ route('events.show', $event) }}" class="flex gap-3 group hover:bg-slate-50 p-2 rounded-xl transition">
                        <!-- ✅ POSTER EVENT (UPDATED) -->
                        <div class="w-16 h-16 rounded-lg flex-shrink-0 overflow-hidden bg-indigo-100">
                            @if($event->poster_path)
                                <img src="{{ asset('storage/' . $event->poster_path) }}" 
                                     alt="{{ $event->title }}" 
                                     class="w-full h-full object-cover">
                            @else
                                <div class="w-full h-full flex items-center justify-center text-indigo-600 font-bold text-xs text-center px-1">
                                    EVENT
                                </div>
                            @endif
                        </div>
