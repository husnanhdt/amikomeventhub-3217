...
@section('page_title', 'Pengawasan Organisasi')
@section('page_subtitle', 'Tinjau, setujui, atau tolak pendaftaran organisasi baru.')
...
<div class="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
            <p class="text-sm text-slate-500 font-medium mb-1">Total Organisasi</p>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
            ...
            <h3 class="text-3xl font-bold text-red-600">{{ $stats['rejected'] }}</h3>
        </div>
    </div>

    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
            <p class="text-sm text-slate-500 font-medium mb-1">Total Event</p>
            <h3 class="text-3xl font-bold text-indigo-600">{{ $stats['total_events'] }}</h3>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between">
        <form method="GET" action="{{ route('superadmin.organizations.index') }}" class="flex-1 min-w-[300px] flex gap-2">
            <input type="text" name="search" placeholder="Cari nama organisasi, email..." 
        ...
        <select onchange="if(this.value) window.location.href=this.value" class="px-4 py-2 rounded-xl border border-slate-200 text-sm font-medium">
            <option value="{{ route('superadmin.organizations.index') }}">Semua Status</option>
            <option value="{{ route('superadmin.organizations.index', ['status' => 'pending']) }}" {{ request('status') == 'pending' ? 'selected' : '' }}>Menunggu</option>
            <option value="{{ route('superadmin.organizations.index', ['status' => 'approved']) }}" {{ request('status') == 'approved' ? 'selected' : '' }}>Disetujui</option>
            <option value="{{ route('superadmin.organizations.index', ['status' => 'rejected']) }}" {{ request('status') == 'rejected' ? 'selected' : '' }}>Ditolak</option>
        </select>
    </div>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="w-12 h-12 bg-indigo-100 rounded-lg flex items-center justify-center">
                                <span class="text-indigo-600 font-bold text-xl">{{ strtoupper(substr($org->name, 0, 1)) }}</span>
                            </div>
                            @endif
                            <div>
                                <div class="font-semibold text-slate-800">{{ $org->name }}</div>
                                @if($org->description)
                                <div class="text-xs text-slate-500 mt-0.5">{{ Str::limit($org->description, 40) }}</div>
                                @endif
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        @if($org->user)
                        <div class="font-medium text-slate-800">{{ $org->user->name }}</div>
                        <div class="text-xs text-slate-500">{{ $org->user->email }}</div>
                        @else
                    ...
                    <td class="px-6 py-4 text-sm text-slate-500">
                        {{ $org->created_at->format('d M Y') }}<br>
                        <span class="text-xs">{{ $org->created_at->format('H:i') }}</span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <!-- Tombol Detail -->
                            <a href="{{ route('superadmin.organizations.show', $org->id) }}" 
                            ...
                            <!-- Tombol Hapus -->
                            <form action="{{ route('superadmin.organizations.destroy', $org->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus organisasi ini?')" class="inline">
                                @csrf
                                @method('DELETE')
